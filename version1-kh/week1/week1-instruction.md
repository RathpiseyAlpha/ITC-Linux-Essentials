...
5. Display the last 15 items in your command history and save it to `history_list.txt`. *(Hint: Use the `>` operator to redirect output. For example: `history 15 > history_list.txt`)*
   (បង្ហាញធាតុ ១៥ ចុងក្រោយនៃប្រវត្តិបញ្ជារបស់អ្នក រួចរក្សាទុកក្នុង `history_list.txt`។ *(ជំនួយ៖ ប្រើប្រាស់សញ្ញា `>` ដើម្បីបង្វែរលទ្ធផលទៅកាន់ឯកសារ។ ឧទាហរណ៍៖ `history 15 > history_list.txt`)*)
